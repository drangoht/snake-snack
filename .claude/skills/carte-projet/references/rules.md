...
| `Rebond.cs` | Les **courbes du juicy** (`docs/art/juicy.md` §2) : `Progres` borné aux deux extrémités (une image longue projetterait un segment au-delà de sa case), `Impulsion` (aller-retour), `Apparition` (pop avec dépassement, **finit exactement à 1**), `Gulp` (compression à surface conservée). ⚠ Rend un facteur, ne décide de rien | `tests/RebondTests.cs` |
